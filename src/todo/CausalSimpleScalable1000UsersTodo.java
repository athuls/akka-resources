...
public class CausalSimpleScalable1000UsersTodo extends UniversalActor  implements ActorService {
	public static void main(String args[]) {
		...
		CausalSimpleScalable1000UsersTodo instance = (CausalSimpleScalable1000UsersTodo)new CausalSimpleScalable1000UsersTodo(uan, ual,null).construct();
		{
	...
	public static ActorReference getReferenceByName(UAN uan)	{ return new CausalSimpleScalable1000UsersTodo(false, uan); }
	public static ActorReference getReferenceByName(String uan)	{ return CausalSimpleScalable1000UsersTodo.getReferenceByName(new UAN(uan)); }
	public static ActorReference getReferenceByLocation(UAL ual)	{ return new CausalSimpleScalable1000UsersTodo(false, ual); }

	public static ActorReference getReferenceByLocation(String ual)	{ return CausalSimpleScalable1000UsersTodo.getReferenceByLocation(new UAL(ual)); }
	public CausalSimpleScalable1000UsersTodo(boolean o, UAN __uan)	{ super(false,__uan); }
	public CausalSimpleScalable1000UsersTodo(boolean o, UAL __ual)	{ super(false,__ual); }

	public CausalSimpleScalable1000UsersTodo(UAN __uan,UniversalActor.State sourceActor)	{ this(__uan, null,null); }
	public CausalSimpleScalable1000UsersTodo(UAL __ual,UniversalActor.State sourceActor)	{ this(null, __ual,null); }
	public CausalSimpleScalable1000UsersTodo(UniversalActor.State sourceActor)		{ this(null, null,null);  }
	public CausalSimpleScalable1000UsersTodo()		{  }
	public CausalSimpleScalable1000UsersTodo(UAN __uan, UAL __ual,Object sourceActor) {
		if (__ual != null && !__ual.getLocation().equals(ServiceFactory.getTheater().getLocation())) {
			createRemotely(__uan, __ual, "todo.CausalSimpleScalable1000UsersTodo");
		} else {
	...
	public class State extends UniversalActor .State implements salsa.resources.ActorServiceState {
		public CausalSimpleScalable1000UsersTodo self;
		public void updateSelf(ActorReference actorReference) {
			((CausalSimpleScalable1000UsersTodo)actorReference).setUAL(getUAL());
			((CausalSimpleScalable1000UsersTodo)actorReference).setUAN(getUAN());
			self = new CausalSimpleScalable1000UsersTodo(false,getUAL());
			self.setUAN(getUAN());
			self.setUAL(getUAL());
			self.muteGC();
		}
		...
		public State(UAN __uan, UAL __ual) {
			super(__uan, __ual);
			addClassName( "todo.CausalSimpleScalable1000UsersTodo$State" );
		...
		public void todoSession(ArrayList users, ArrayList userNames, int num_users, int num_messages, String server_ref, String logDirectory, TaskList taskList) {
			...
			for (int i = 0; i<200; i++){
				...
				for (int j = 1; j<=num_messages; j++){
					...
					int userIndex = i+1;
					taskMessage = "User"+userIndex+": Initial Task "+values.get(j-1)+".";
					{
			...
			for (int i = 200; i<users.size(); i++){
				...
				for (int j = 1; j<=num_messages; j++){
					...
					taskMessage = "User"+(i+1)+": Follow Up Task "+values.get(j-1)+".";
					{
						// registeredUser<-addTaskToList(taskList, taskMessage, (String)userNames.get(i), taskname, isFollowUp, num_messages, (num_users-1)*num_messages, true)
						{
							Object _arguments[] = { taskList, taskMessage, (String)userNames.get(i), taskname, isFollowUp, num_messages, (num_users-1)*num_messages, true };
